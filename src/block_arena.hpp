...
public:
    ...
    size_t metadata_capacity_bytes() const {
        return sizeof(BlockArena) + blocks.capacity() * sizeof(StoredBlock)
            + cblocks.capacity() * sizeof(CBlock);
    }
    size_t palette_capacity_bytes() const {
        size_t bytes = 0;
        for (const auto& block : blocks) bytes += block.palette_bytes.capacity();
        return bytes;
    }
    size_t bitstream_capacity_bytes() const {
        size_t bytes = 0;
        for (const auto& block : blocks)
            bytes += block.bitstream.capacity() * sizeof(uint32_t);
        return bytes;
    }
