...
void ExtractBlockMetadata(
    ...
    size_t voxels_per_block = 1;
    size_t total_blocks = 1;

    for (size_t i = 0; i < 3; ++i) {
        if (volume_size[i] <= 0 || block_size[i] <= 0 ||
            intersection_start[i] < 0 ||
            intersection_start[i] > intersection_end[i] ||
            intersection_end[i] > volume_size[i]) {
            throw std::runtime_error("invalid compressed segmentation dimensions");
        }
        grid_size[i] = (volume_size[i] + block_size[i] - 1) / block_size[i];
        total_blocks *= static_cast<size_t>(grid_size[i]);
        voxels_per_block *= static_cast<size_t>(block_size[i]);
        block_start[i] = intersection_start[i] / block_size[i];
        block_end[i] = (intersection_end[i] + block_size[i] - 1) / block_size[i];
    }
    if (total_blocks > input_word_count / 2) {
        throw std::runtime_error("compressed segmentation block headers exceed input buffer");
    }
    ...
    for (block[2] = block_start[2]; block[2] < block_end[2]; ++block[2]) {
        for (block[1] = block_start[1]; block[1] < block_end[1]; ++block[1]) {
            for (block[0] = block_start[0]; block[0] < block_end[0]; ++block[0]) {
                ...
                if (!(bits == 0 || bits == 1 || bits == 2 || bits == 4 ||
                      bits == 8 || bits == 16)) {
                    throw std::runtime_error("invalid compressed segmentation bit width");
                }
                const size_t bitstream_words = (voxels_per_block * bits + 31) / 32;
                if (encoded_value_start > input_word_count ||
                    bitstream_words > input_word_count - encoded_value_start) {
                    throw std::runtime_error("compressed segmentation bitstream exceeds input buffer");
                }

                // The format stores the encoding capacity in `bits`, not the
                // number of palette entries. Recover the latter from the
                // largest index that the block's bitstream actually uses.
                size_t palette_size = 1;
                if (bits != 0) {
                    const uint32_t* bitstream = input + encoded_value_start;
                    const uint32_t mask = (uint32_t(1) << bits) - 1;
                    uint32_t maximum_index = 0;
                    for (size_t voxel = 0; voxel < voxels_per_block; ++voxel) {
                        const size_t bit_position = voxel * bits;
                        const size_t word_index = bit_position / 32;
                        const size_t bit_shift = bit_position % 32;
                        uint32_t index = (bitstream[word_index] >> bit_shift) & mask;
                        if (bit_shift + bits > 32) {
                            index |= (bitstream[word_index + 1] << (32 - bit_shift)) & mask;
                        }
                        maximum_index = std::max(maximum_index, index);
                    }
                    palette_size = static_cast<size_t>(maximum_index) + 1;
                }
                const size_t words_per_label =
                    (sizeof(Label) + sizeof(uint32_t) - 1) / sizeof(uint32_t);
                const size_t palette_words = palette_size * words_per_label;
                if (table_offset > input_word_count ||
                    palette_words > input_word_count - table_offset) {
                    throw std::runtime_error("compressed segmentation palette exceeds input buffer");
                }
                ...
                info.bitstream_word_count = bitstream_words;
...
// 模板显式实例化
template void ExtractBlockMetadata<uint8_t>(const uint32_t*, size_t, const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], std::vector<BlockInfo<uint8_t>>&);
template void ExtractBlockMetadata<uint32_t>(const uint32_t*, size_t, const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], std::vector<BlockInfo<uint32_t>>&);
template void ExtractBlockMetadata<uint64_t>(const uint32_t*, size_t, const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], const ptrdiff_t[3], std::vector<BlockInfo<uint64_t>>&);
