...
class ShardReader(object):
  ...
  def get_indices(self, filenames, path="", progress=None, return_info=False):
    """
    ...
    get_indices_info['get_indices_filename_prepare'] = time.time() - total_start_time
    start_time = time.time()
    progress = 'Shard Indices' if progress else False
    if return_info:
      binaries, cacheserver_info = self.cache.download_as(requests, progress=progress, return_info=True)
    else:
    ...
    if return_info:
      ...
      get_indices_info['get_indices_download_as_cache_hit_num'] = cacheserver_info['download_as_cache_hit_num']
      get_indices_info['get_indices_download_as_cache_miss_num'] = cacheserver_info['download_as_cache_miss_num']
      get_indices_info['get_indices_download_as_remote_time'] = cacheserver_info['download_as_remote_time']
      get_indices_info['get_indices_download_as_remote_size'] = cacheserver_info['download_as_remote_size']
      return fufilled, get_indices_info
    return fufilled
  ...
  def get_minishard_indices_for_files(self, requests, path="", progress=None, return_info=False):
    """
    ...
    get_minishard_indices_info['get_minishard_indices_filename_prepare'] = time.time() - total_start_time

    start_time = time.time()
    progress = 'Minishard Indices' if progress else False
    if return_info:
      results, cacheserver_info = self.cache.download_as(download_requests, progress=progress, return_info=True)
    else:
      results = self.cache.download_as(download_requests, progress=progress)
      cacheserver_info = {}
    get_minishard_indices_info['get_minishard_indices_download_time'] = time.time() - start_time

    total_bytes = sum(len(content) for content in results.values() if content is not None)
    get_minishard_indices_info['get_minishard_indices_download_size'] = total_bytes
    start_time = time.time()
    ...
    get_minishard_indices_info['get_minishard_indices_gzip_decode'] = time.time() - start_time
    get_minishard_indices_info['get_minishard_indices_total_time'] = time.time() - total_start_time
    get_minishard_indices_info['get_indices_total_time'] = time.time() - total_start_time
    if return_info:
      get_minishard_indices_info['get_minishard_indices_download_as_cache_get_time'] = cacheserver_info['download_as_cache_get_time']
      get_minishard_indices_info['get_minishard_indices_download_as_cache_put_time'] = cacheserver_info['download_as_cache_put_time']
      get_minishard_indices_info['get_minishard_indices_download_as_cache_hit_num'] = cacheserver_info['download_as_cache_hit_num']
      get_minishard_indices_info['get_minishard_indices_download_as_cache_miss_num'] = cacheserver_info['download_as_cache_miss_num']
      get_minishard_indices_info['get_minishard_indices_download_as_remote_time'] = cacheserver_info['download_as_remote_time']
      get_minishard_indices_info['get_minishard_indices_download_as_remote_size'] = cacheserver_info['download_as_remote_size']
      return fufilled_by_filename, get_minishard_indices_info
  ...
  def exists(self, labels, path="", return_byte_range=False, progress=None, return_info=False):
    """
    ...
    exists_info['exists_info_filename_prepare' ] = time.time() - total_start_time
    start_time = time.time()
    indices, get_indices_info = self.get_indices(to_all_labels.keys(), path, progress=progress, return_info=True)
    exists_info['exists_info_get_indices_time' ] = time.time() - start_time

    start_time = time.time()
    all_minishards, get_minishard_indices_info = self.get_minishard_indices_for_files([
        (basename(filepath), index, filename_to_minishard_num[basename(filepath)]) \
        for filepath, index in indices.items()
    ], path, progress=progress, return_info=True)
    exists_info['exists_info_get_minshard_indices_time'] = time.time() - start_time
    start_time = time.time()
    results = {}
    ...
    if return_one:
      ...
      return (result, exists_info) if return_info else result
    return (results, exists_info) if return_info else results
  ...
  def get_data(
    ...
    raw:bool = False, cache_thread=None, return_info=False,
  ):
    ...
    cache_thread: num of threads to read cache, 0 means
    Return:
    ...
    total_requested_labels = len(label)
    start_time = time.time()
    cached = {}
    if self.cache.enabled:
      if cache_thread is None:
        ...
        cached = self.cache.optional_get([
          self.meta.join(path, str(lbl)) for lbl in label
        ], num_threads=cache_thread, progress=progress)
    ...
    exists, exist_info = self.exists(label, path, return_byte_range=True, progress=progress, return_info=True)
    get_data_info['get_data_info_exists_time'] = time.time() - start_time
    ...
    get_data_info['get_data_info_build_download_request'] = time.time() - start_time
    ...
    get_data_info['get_data_info_download_chunk_time'] = time.time() - start_time

    total_bytes = sum(len(resp['content']) for resp in bundles_resp if resp.get('content'))

    get_data_info['get_data_info_download_size'] = total_bytes
    ...
    get_data_info['get_data_info_gzip_decode'] = time.time() - start_time
    ...
    get_data_info['get_data_info_cache_update'] = time.time() - start_time

    results.update(binaries)
    get_data_info['get_data_info_total_time'] = time.time() - total_start_time
    get_data_info['get_data_info_disk_hits'] = disk_hits
    get_data_info['get_data_info_disk_hits_miss'] = total_requested_labels - disk_hits
    get_data_info['get_data_info_total_requested'] = total_requested_labels
    get_data_info.update(exist_info)
    result = results if return_multiple else first(results.values())
    return (result, get_data_info) if return_info else result
