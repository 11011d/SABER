...
except ImportError: # optional SABER extension path
  CompressedVoxelContainer = None
...
def download_sharded_compressed_block(
  ...
  label, cache_thread=None, segid_list=None, return_info=False
):
  if CompressedVoxelContainer is None:
    raise ImportError("saber=True requires compressedvoxel.CompressedVoxelContainer")
  if label is not None:
    raise ValueError("Compressed-block SABER reads do not support label reads.")
  ...
  if not meta.overlaps_roi(requested_bbox, mip):
    result = None
    return (result, download_sharded_info) if return_info else result
  ...
  io_chunkdata, get_data_info = reader.get_data(io_keys, meta.key(mip), cache_thread=cache_thread, progress=progress, return_info=True)
  ...
  return (container, download_sharded_info) if return_info else container
...
def download_sharded(
  ...
  if not meta.overlaps_roi(requested_bbox, mip):
    return (renderbuffer, download_sharded_info) if return_info else renderbuffer
  ...
  start_time_io = time.time()
  if return_info:
    io_chunkdata, get_data_info = reader.get_data(
      io_keys, meta.key(mip), progress=progress, return_info=True
    )
  else:
  ...
  for zcode, (data_encoding, chunkdata) in itertools.chain(io_chunkdata.items(), lru_chunkdata):
    ...
    time_decode += time.time() - decode_start
  ...
  download_sharded_info['requested_bbox'] = [requested_bbox.minpt, requested_bbox.maxpt]
  download_sharded_info.update(get_data_info)
  result = VolumeCutout.from_volume(
    meta, mip, renderbuffer,
    requested_bbox
  )
  return (result, download_sharded_info) if return_info else result

def download_sharded_saber(
  requested_bbox, mip,
  meta, cache, lru, lru_encoding, spec,
  compress, progress,
  fill_missing, 
  order, background_color,
  label, partial_decompress_parallel=None,
  renderbuffer=None, cache_thread=None, l2cache_size=0, return_info=False
):
  total_start_time = time.time()
  download_sharded_info = {}
  download_sharded_info['download_sharded_requested_bbox_size'] = requested_bbox.size3()
  ...
  effective_partial_decompress_parallel = partial_decompress_parallel
  if label is not None:
    decode_fn = partial(decode_binary_image, label)
  elif single_voxel:
    decode_fn = partial(decode_single_voxel, requested_bbox.minpt - full_bbox.minpt)
    ...
    if partial_decompress_parallel is None:
      ...
      effective_partial_decompress_parallel = 1
    else:
      decode_fn = decode_partial_parallel
      use_decode_partial_parallel = True
  download_sharded_info['download_sharded_decode_fn_init'] = time.time() - start_time
  ...
  if use_decode_partial_parallel:
    ...
    decode_fn(
      ...
      fill_missing, mip,
      background_color=background_color,
      partial_decompress_parallel=effective_partial_decompress_parallel,
