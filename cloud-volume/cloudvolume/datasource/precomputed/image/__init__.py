...
class PrecomputedImageSource(ImageSourceInterface):
  ...
  def download(
    ...
    renderbuffer = None, cache_thread = None,
    l2cache_size = 0, use_compressed_block = False,
    segid_list = None
  ):
    """
    ...
    partial_decompress_parallel: If provided, use partial decompress
      parallel to speedup
    ...
    if self.is_sharded(mip):
      ...
      if use_compressed_block:
        return rx.download_sharded_compressed_block(
          ...
          cache_thread = cache_thread, segid_list = segid_list
        )
      else:
