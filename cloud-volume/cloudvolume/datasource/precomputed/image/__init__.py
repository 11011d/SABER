...
class PrecomputedImageSource(ImageSourceInterface):
  ...
  def download(
    ...
    saber=None, renderbuffer=None, cache_thread=None,
    saber_debug=False,
    segid_list=None, _saber_return_info=False, saber_options=None
  ):
    """
    ...
    saber: True returns compressed blocks. An integer enables dense partial
      decompression with that parallelism value.
    cache_thread: num of threads in threads pool to read cache, 0 means cancel pool, 
      None means use default config in cloud-files
    saber_debug: return detailed SABER debug information.
    Returns:
      if renumber:
    ...
    if self.is_sharded(mip):
      ...
      if saber_options is None and (
        saber is not None
        or cache_thread is not None
        or saber_debug
      ):
        saber_options = SaberOptions.from_params(
          saber=saber,
          cache_thread=cache_thread,
          saber_debug=saber_debug,
        )
      if saber_options is not None and saber_options.enabled:
        _saber_return_info = _saber_return_info or saber_options.debug
        if saber_options.level == 2 and label is not None:
          raise ValueError("SABER partial decompression does not support label reads.")
        if saber_options.level == 3 and label is not None:
          raise ValueError("SABER compressed-block reads do not support label reads.")
        if saber_options.level == 3:
          return rx.download_sharded_compressed_block(
            bbox, mip,
            ...
            label=label,
            cache_thread=saber_options.cache_thread,
            segid_list=segid_list,
            return_info=_saber_return_info,
          )
        return rx.download_sharded_saber(
          bbox, mip,
          self.meta, self.cache, self.lru, self._lru_encoding, spec,
          compress=self.config.compress,
          progress=progress,
          ...
          background_color=numberfn(self.background_color),
          label=label,
          partial_decompress_parallel=saber_options.partial_decompress_parallel,
