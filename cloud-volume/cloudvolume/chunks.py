...
def _compressed_segmentation():
  check_installed('compressed_segmentation')
  return cseg
...
def decode_compressed_segmentation(bytestring, shape, dtype, block_size):
  ...
  return _compressed_segmentation().decompress(bytes(bytestring), shape, dtype, block_size, order='F')

def partial_decompress_in_place_parallel(
  requests: list, partial_decompress_parallel: int, order='F', l2cache_size: int = 0
):
  """
  Partial in-place decompression using the Cython binding.
  """
  return _compressed_segmentation().decompress_partial_in_place_parallel(
    requests=requests,
    parallel=partial_decompress_parallel,
    order=order,
    l2cache_size=l2cache_size
  )

def partial_decompress(filedata, background_color, encoding, shape, dtype, block_size, chunk_start, chunk_end, request_start, request_end):
  if (shape is None or dtype is None) and encoding not in ('npz', 'fpzip', 'kempressed', 'crackle', 'compresso'):
    raise ValueError(
      f"Only npz, fpzip, kempressed, crackle, and compresso encoding "
      f"can omit shape and dtype arguments. Got: {encoding}"
  ...
  result, intersection_start, intersection_end = _compressed_segmentation().decompress_partial(bytes(filedata), shape, dtype, chunk_start, chunk_end, request_start, request_end, block_size, order='F')
  return result

def decode_partial_compressed_block(filedata, background_color, encoding, shape, dtype, block_size, chunk_start, chunk_end, request_start, request_end, chunk_relative_grid, container, segid_list):
  if (shape is None or dtype is None) and encoding not in ('npz', 'fpzip', 'kempressed', 'crackle', 'compresso'):
    raise ValueError(
      f"Only npz, fpzip, kempressed, crackle, and compresso encoding "
      f"can omit shape and dtype arguments. Got: {encoding}"
  ...
  block_per_chunk = np.array(shape[:3]) // np.array(block_size)
  offset_grid = np.array(chunk_relative_grid) * block_per_chunk
  _compressed_segmentation().extract_to_container(
      bytes(filedata), shape, dtype, 
      chunk_start, chunk_end, 
      request_start, request_end, 
      container=container,
      chunk_relative_grid=offset_grid,
