...
def partial_decompress_in_place_parallel(
  ...
  """
  # print(f"using decompress_partial_in_place_parallel!l2cache_size={l2cache_size}")
  return cseg.decompress_partial_in_place_parallel(
    requests=requests,
...
def partial_decompress(filedata, background_color, encoding, shape, dtype, block_size, chunk_start, chunk_end, request_start, request_end):
  ...
  result, intersection_start, intersection_end = cseg.decompress_partial(bytes(filedata), shape, dtype, chunk_start, chunk_end, request_start, request_end, block_size, order='F')
  return result
...
def decode_partial_compressed_block(filedata, background_color, encoding, shape, dtype, block_size, chunk_start, chunk_end, request_start, request_end, chunk_relative_grid, container, segid_list):
  ...
  # print(f"decode_partial_compressed_block segid_list={segid_list}")
