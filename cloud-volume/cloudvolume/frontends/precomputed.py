...
class CloudVolumePrecomputed(object):
  def __init__(self, 
    ...
    mip=0, log_path = None, partial_decompress_parallel=None, 
    renderbuffer = None, cache_thread = None, l2cache_size =0, use_compressed_block=False, segid_list = None
  ):
    self.log_path = log_path 
    self.renderbuffer = renderbuffer
    self.l2cache_size = l2cache_size
    self.use_compressed_block=use_compressed_block
    # partial_decompress_parallel == 1 means use partial_decompress, partial_decompress_parallel > 1 means use parallel to speedup
    self.partial_decompress_parallel = partial_decompress_parallel
    self.config = config 
    self.cache = cache 
    self.meta = meta
  ...
  def _log_info(self, info_dict: dict):
    ...
    try:
        ...
        info_str = json.dumps(info_dict, default = self.json_converter)
        log_line = f"{timestamp} - {info_str}\n"
        with open(self.log_path, 'a', encoding='utf-8') as f:
            f.write(log_line)
    except Exception as e:
        print(f"Cloudvolume Warning: Failed to write to log file {self.log_path}. Error: {e}")
  ...
  def __getitem__(self, slices):
    ...
    img, info = self.download(requested_bbox, self.mip)
    # time_start = time.time()
    # print(f"save cloudvolume info cost time = {time.time() - time_start}")
    info ["total_time_of_all"] = time.time() - time_start
    self._log_info(info)
    # return img[::steps.x, ::steps.y, ::steps.z, channel_slice],info
    if self.use_compressed_block:
      return img
    else:
  ...
  def download(
    ...
    tup, info = self.image.download(
      bbox, mip, 
      parallel=parallel, 
      renumber=bool(renumber),
      label=label,
      partial_decompress_parallel = self.partial_decompress_parallel,
      renderbuffer = self.renderbuffer,
      cache_thread = self.cache_thread,
      l2cache_size = self.l2cache_size,
      use_compressed_block=self.use_compressed_block,
