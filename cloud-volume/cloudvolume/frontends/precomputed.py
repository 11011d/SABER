...
from ..saber import SaberOptions
...
class CloudVolumePrecomputed(object):
  def __init__(self, 
    ...
    mip=0, renderbuffer=None, segid_list=None, saber_options=None
  ):
    self.config = config
    self.cache = cache
    self.meta = meta
    ...
    if saber_options is None:
      saber_options = SaberOptions.disabled()
    self.saber_options = saber_options
    self.partial_decompress_parallel = saber_options.partial_decompress_parallel
    self.renderbuffer = renderbuffer
    self.cache_thread = saber_options.cache_thread
    self.use_compressed_block = saber_options.use_compressed_block
  ...
  def _download_with_info(self, bbox, mip, **kwargs):
    return self.download(bbox, mip, _saber_return_info=True, **kwargs)

  def _uses_saber_path(self, mip):
    return (
      self.saber_options.enabled
      and self.image.is_sharded(mip)
      and self.meta.encoding(mip) == "compressed_segmentation"
    )

  def _supports_sharded_debug_info(self, mip):
    return (
      self.image.is_sharded(mip)
      and self.meta.encoding(mip) == "compressed_segmentation"
    )

  def distinct_labels_in_mask(self, bbox, selection, mip=None):
    """Return labels under a query-local mask without materializing a dense cutout."""
    if mip is None:
      mip = self.mip
    mip = self.meta.to_mip(mip)
    if not self._uses_saber_path(mip) or self.saber_options.level != 3:
      raise ValueError(
        "distinct_labels_in_mask requires saber=True on sharded compressed_segmentation"
      )
    if isinstance(bbox, Bbox):
      bbox = bbox.convert_units("vx", self.meta.resolution(mip)).astype(int)
    elif (
      isinstance(bbox, (tuple, list))
      and len(bbox) == 2
      and np.asarray(bbox[0]).shape == (3,)
      and np.asarray(bbox[1]).shape == (3,)
    ):
    ...
    expected_shape = tuple(int(value) for value in bbox.size3())
    if selection.dtype != np.bool_:
      raise TypeError("selection must be a boolean array")
    if selection.shape != expected_shape:
      raise ValueError(
        f"selection shape {selection.shape} does not match bbox {expected_shape}"
      )

    fetch_started = time.perf_counter()
    container = self.download(bbox, mip)
    fetch_seconds = time.perf_counter() - fetch_started
    select_started = time.perf_counter()
    labels, stats = container.distinct_labels_in_mask(selection)
    select_seconds = time.perf_counter() - select_started
    return labels, {
      **stats,
      "fetch_seconds": fetch_seconds,
      "select_seconds": select_seconds,
      "bbox_voxels": int(bbox.volume()),
    }

  def discover_labels(self, bbox, selection, mip=None):
    """Semantic query alias for label discovery under a spatial predicate."""
    return self.distinct_labels_in_mask(bbox, selection, mip=mip)
  ...
  def __getitem__(self, slices):
    ...
    return_info = (
      self.saber_options.debug
      and self._supports_sharded_debug_info(self.mip)
    )
    if return_info:
      img, info = self._download_with_info(requested_bbox, self.mip)
      info["total_time_of_all"] = time.time() - time_start
    else:
      img = self.download(requested_bbox, self.mip)

    if self.saber_options.level == 3 and self._uses_saber_path(self.mip):
  ...
  def download(
    ...
    use_saber = self._uses_saber_path(mip)
    return_info = _saber_return_info or (
      self.saber_options.debug and self._supports_sharded_debug_info(mip)
    )
    if return_info and not self._supports_sharded_debug_info(mip):
      raise ValueError("Debug info is only supported for sharded compressed_segmentation reads.")
    ...
    tup = self.image.download(
      ...
      segid_list=self.segid_list,
      _saber_return_info=return_info,
      saber_options=(self.saber_options if use_saber else None),
    )
    ...
    if segids is None:
      return (tup, info) if return_info else tup
    mask_value = 0
    if preserve_zeros:
      mask_value = np.inf
      if np.issubdtype(self.dtype, np.integer):
    ...
    result = (img, remap) if renumber else img
