...
class CacheService(object):
  ...
  def download_as(self, requests, compress=None, progress=None, return_info=False):
    """
    ...
    if len(requests) == 0:
      info = {
        'download_as_cache_get_time': 0,
        'download_as_cache_put_time': 0,
        'download_as_cache_hit_num': 0,
        'download_as_cache_miss_num': 0,
        'download_as_remote_time': 0,
        'download_as_remote_size': 0,
      }
    ...
    if self.enabled:
      ...
      for alias in locs['local']:
        cache_hit_num = cache_hit_num + 1
        del alias_tuples[alias]
      cache_get_time = time.time() - time_start
    ...
    remote_num = len(remote_path_tuples)
    ...
    total_bytes = sum(len(content) for content in remote_fragments.values() if content is not None)
    remote_time = time.time() - time_start
    if self.enabled:
      time_start = time.time()
      self.put(
        [ 
      ...
      cache_put_time = time.time() - time_start
    fragments.update(remote_fragments)
    cacheserver_info = {
      'download_as_cache_get_time': cache_get_time,
      'download_as_cache_put_time': cache_put_time,
      'download_as_cache_hit_num': cache_hit_num,
      'download_as_cache_miss_num': remote_num,
      'download_as_remote_time': remote_time,
      'download_as_remote_size': total_bytes,
    }
    return (fragments, cacheserver_info) if return_info else fragments
  ...
  def optional_get(self, cloudpaths, num_threads=0, progress=None):
    if num_threads == 1:
      return self.get(cloudpaths, progress=progress)
    progress = self.config.progress if progress is None else progress
