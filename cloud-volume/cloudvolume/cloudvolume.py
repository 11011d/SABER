...
class CloudVolume:
  def __new__(cls,
    ...
    timestamp:Optional[int] = None, saber=None, cache_thread:Optional[int] = None,
    saber_debug:bool = False,
  ):
    """
    ...
    Optional:
      ...
      saber: (bool or int) compact SABER read selector for sharded
        compressed_segmentation volumes. True returns compressed blocks, an
        integer enables dense partial decompression with that parallelism value,
        and False/None leaves those modes disabled.
      cache_thread: (int) optional thread count for local cache reads in SABER
        paths. Passing cache_thread without saber enables cache-read-only SABER
        mode.
      saber_debug: (bool) return detailed SABER debug info with reads that use a
        SABER path.
    """
    if use_https:
